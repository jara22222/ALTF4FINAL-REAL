...
if (!empty($_GET['search_query'])) {
  ...
  $stmt = $conn->prepare("SELECT * FROM adds_on WHERE ADID LIKE ? OR add_name LIKE ? OR category_name LIKE ? LIMIT ? OFFSET ?");
  $stmt->bind_param("sssii", $searchQuery, $searchQuery, $searchQuery, $limit, $offset);
  $stmt->execute();
  $result = $stmt->get_result();

  // Count total rows
  $stmtTotal = $conn->prepare("SELECT COUNT(*) FROM adds_on WHERE ADID LIKE ? OR add_name LIKE ? OR category_name LIKE ?");
  $stmtTotal->bind_param("sss", $searchQuery, $searchQuery, $searchQuery);
  $stmtTotal->execute();
  $stmtTotal->bind_result($totalRows);
  $stmtTotal->fetch();
  $stmtTotal->close();
} else {
  $result = $conn->query("SELECT *,c.category_name FROM adds_on a 
  JOIN categories c on a.CID = c.CID ORDER BY ADID LIMIT $limit OFFSET $offset");
  $totalRowsResult = $conn->query("SELECT COUNT(*) AS total FROM adds_on");
...
<div class="d-flex flex-column col main-content">
    <div class="col p-4 content">
        ...
        <?php if (isset($_SESSION['errors'])): ?>
            ...
            <div class="col d-flex align-items-center justify-content-between p-0">
                <h3 class="mt-3 page-title">Manage Add_Ons</h3>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col d-flex align-items-center justify-content-between p-0">
            ...
            <form class="d-flex align-items-center" method="GET" action="Add_ons.php">
            ...
            <div class="d-flex align-items-center justify-content-end gap-3">
                <div class="d-flex align-items-center">
                    ...
                    data-bs-target="#infoModal2">
                        ...
                        <span class="text-center">Add add_ons</span>
                    </button>
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="infoModalLabel2">Add Add-On</h5>
                          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                          <div class="row">
                            <form action="../HANDLERS/addAdd_ons_handler.php" method="POST">
                              <div class="col-12">
                                <label for="add_name">Add-On Name:</label>
            ...
            <?php if ($result->num_rows > 0) : ?>
                ...
                <tr>
                    ...
                    <td> <button type="button" class="btn btn-info btn-sm"
                            data-bs-toggle="modal"
                            data-bs-target="#infoModal<?php echo $row['ADID']; ?>">
                            See More&nbsp;<i class="fa-solid fa-ellipsis"></i>
                        </button></td>
                    <td>
                        <form action="../Handlers/deleteAdd_ons_handler.php" method="POST"
                                onsubmit="return confirmUpdate2()">
                                <button type="button" class="btn btn-warning btn-sm"
                                data-bs-target="#editModal<?php echo $row['ADID']; ?>" data-bs-toggle="modal">

                                    <i class="fa-solid fa-pen"></i>
                                </button>
                                <input type="hidden" name="CID"
                                    value="<?php echo htmlspecialchars($row['ADID']); ?>">
                 ...
                 <!-- Info Modal -->
                 <div class="modal fade" id="infoModal<?php echo $row['ADID']; ?>" tabindex="-1"
                            aria-labelledby="infoModalLabel<?php echo $row['ADID']; ?>" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                              <div class="modal-content">
                                <div class="modal-header">
                                  <h5 class="modal-title" id="infoModalLabel<?php echo $row['ADID']; ?>">Add-On Details</h5>
                                  ...
                                  <div class="row">
                                    <div class="col-12">
                                      <h5 class="mt-3 mb-4 text-center">
                                        <?php echo htmlspecialchars($row['add_name']); ?>
                                      </h5>
                                      <p><strong>Category:</strong> <?php echo htmlspecialchars($row['category_name']); ?>
                                      </p>
                                      <p><strong>Price:</strong> ₱<?php echo number_format($row['price'], 2); ?></p>
                                      <p><strong>Weight:</strong> <?php echo htmlspecialchars($row['weight']); ?>g</p>
                                    </div>
                                  </div>
                                </div>
                                <div class="modal-footer">
                          ...
                          <!-- Update Modal -->
                          <div class="modal fade edit-modal" id="editModal<?php echo $row['ADID']; ?>" tabindex="-1"
                            aria-labelledby="editModalLabel<?php echo $row['ADID']; ?>" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                              <div class="modal-content">
                                <div class="modal-header">
                                  <h5 class="modal-title">Update Add-On</h5>
                                  <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                  <form action="../HANDLERS/updateAdd_ons_handler.php" method="POST"
                                    onsubmit="return confirmUpdate()">
                                    <input type="hidden" name="ADID" value="<?php echo $row['ADID']; ?>">

                                    <label for="add_name">Add-On Name:</label>
                                    <input type="text" id="add_name" name="add_name" class="form-control"
                                      value="<?php echo htmlspecialchars($row['add_name']); ?>" required>
    ...
    <!-- CUSTOM JS -->
        <script>
            function confirmUpdate() {
                return confirm("Are you sure you want to update this add-ons's information?");
            }
            function confirmUpdate2() {
                return confirm("Are you sure you want to delete this add-ons's information?");
            }
