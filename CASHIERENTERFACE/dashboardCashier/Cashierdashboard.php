...
<body>
            ...
            <div class="col-md-8 content">
                ...
                <div class="col-md-3 order-panel p-3">
                    <div class="container p-0 gap-2 mt-3 m-0">
                        <p class="cashierID">Cashier ID: CAS-0001-0225</p> <!--ILISAN NI SA DATABASE-->
                    </div>
                    <div class="container p-0 gap-2">
                        <p class="cashierName">Cashier Name: Disney Princess</p> <!--ILISAN NI SA DATABASE-->
                    </div>
                    <div class="">
                                    ...
                                    document.addEventListener('DOMContentLoaded', function () {
                                        ...
                                        document.querySelectorAll('.modal').forEach(modal => {
                                            ...
                                            modal.addEventListener('click', function (event) {
                                                ...
                                                if (target.classList.contains('size-btn')) {
                                                    ...
                                                    selectedSizePrice = parseFloat(target.dataset.price);
                                                }
                                                ...
                                                if (target.classList.contains('addon-btn')) {
                                                    ...
                                                    let addonPrice = parseFloat(target.dataset.price);

                                                    if (target.classList.contains('active')) {
                                                        if (!selectedAddOns.some(addon => addon.name === addonName)) {
                                                ...
                                                if (target.classList.contains('add-order-btn')) {
                                                    ...
                                                    let isPastry = target.closest('.modal').id.includes('PastryfoodModal');
                                                    let isMeal = target.closest('.modal').id.includes('RiceMealfoodModal');
                                                    let finalPrice = selectedSizePrice;

                                                    if (isPastry || isMeal) {
                                                        finalPrice = parseFloat(target.dataset.price) || 0;
                                                    } else {
                                                        if (!selectedSize) {
                                                    ...
                                                    console.log(`Adding ${productName} - Price: ₱${finalPrice}`);

                                                    addToOrder(productName, finalPrice, selectedSize, selectedAddOns);

                                                    // ✅ Close modal safely
                                                    let modalElement = document.getElementById(`DrinksfoodModal${productId}`) ||
                                                        document.getElementById(`PastryfoodModal${productId}`) ||
                                                        document.getElementById(`RiceMealfoodModal${productId}`);
                                        ...
                                        function addToOrder(itemName, itemPrice, size = '', addOns = []) {
                                            ...
                                            if (existingItem) {
                                                ...
                                                orderItems.push({ key: itemKey, name: itemName, price: itemPrice, qty: 1, size });
                                            }

                                            // ✅ Add add-ons correctly
                                            addOns.forEach(addon => {
                                                let addonKey = `${itemKey}-addon-${addon.name}`;
                                                let existingAddon = orderAddOns.find(a => a.key === addonKey);

                                                if (existingAddon) {
                                                    existingAddon.qty += 1;
                                                } else {
                                                    orderAddOns.push({ key: addonKey, name: addon.name, price: addon.price, qty: 1, parentKey: itemKey });
                                                }
                                            });
    ...
    <input type="hidden" value="${item.qty}" min="1" name="qty${item.qty}" id="qty${item.qty}">
            ...
            `;
                                                ...
                                                orderItems.forEach(item => {
                                                    productNames.push(item.name); // Collect product name
                                                    productSizes.push(item.size); // Collect product size
                                                    productQty.push(item.qty); // Add quantity to the total
                                                });

                                                // Iterate through orderAddOns to collect add-on data
                                                orderAddOns.forEach(addon => {
                                                    addonNames.push(addon.name); // Collect add-on name
                                                    addonQty.push(addon.qty); // Add add-on quantity to the total
                                                });

                                                // Now, populate the hidden fields with the collected data
                                                document.getElementById('hiddenProducts').value = productNames.join(', ');  // Join product names with commas
                                                document.getElementById('hiddenQty').value = productQty.join(', ');  // Total quantity of products
                                                document.getElementById('size').value = productSizes.join(', ');  // Join product sizes with commas

                                                document.getElementById('hiddenaddons').value = addonNames.join(', ');  // Join add-on names with commas
                                                document.getElementById('hiddenQtyaddons').value = addonQty.join(', ');  // Total quantity of add-ons
    ...
    <input type="hidden" value="${addon.qty}" name="size${addon.qty}" id="size${addon.qty}">
            ...
            `;
                                            ...
                                            document.querySelector('.subtotalAmount').innerText = `₱${subtotal.toFixed(2)}`;
                    ...
                    <div class="container-date-time-name d-flex gap-2 mb-3">
                        <p class="store-name m-0">Blacksnow Cafe</p> <!--ILISAN NI SA DATABASE-->
                        <p class="m-0">|</p>
                        <p class="order-date m-0">Date</p> <!--ILISAN NI SA DATABASE-->
                        <p class="m-0">|</p>
                        <p class="order-time m-0">Time</p> <!--ILISAN NI SA DATABASE-->
                    </div>
                    ...
                    <div class="container-total p-0">
                        <div class="d-flex justify-content-between align-items-center mt-2">
                            ...
                            <p class="subtotalAmount mb-0"></p> <!--ILISAN NI SA DATABASE-->
                        </div>
    ...
    <div class="form-check d-flex align-items-center">
        ...
        <div class="inputPaymentCash gap-2 d-flex align-items-center">
            ...
            <input class="cashchange form-control ms-2" type="text" placeholder="Change" readonly>
        </div>
    </div>
...
<div class="d-flex justify-content-center mt-4 gap-3 mb-4">
    ...
    <button type="button" class="btn btn-success" onclick="payNowClicked()" data-bs-toggle="modal">
        Pay Order Now
    </button>
</div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary" onclick="printReceipt()">Print Receipt</button>
                    </form>
                </div>
            </div>
        </div>
...
function payNowClicked() {
    ...
    orderTableBody?.querySelectorAll("tr").forEach(row => {
        ...
        if (itemNameElem && itemQtyElem && itemPriceElem) {
            ...
            let itemQty = itemQtyElem.tagName === "INPUT" ? parseInt(itemQtyElem.value) || 1 : parseInt(itemQtyElem.innerText) || 1;
            ...
            orderItems.push({ name: itemName, size: itemSize, qty: itemQty, price: itemPrice, isAddon });
        }
    });
